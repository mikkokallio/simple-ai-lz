...
export default function DocumentViewer({ document, onReset }: Props) {
  ...
  const finalizeDocument = async () => {
    ...
    try {
      ...
      setError(`${t('errorFinalizing')}: ${err.message}`);
      console.error(err);
    } finally {
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="card mb-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">{t('documentCreated')}</h2>
            <p className="text-gray-600 mt-1">
              {isFinalized ? (
                <span className="text-green-600 font-medium">✓ {t('finalized')}</span>
              ) : (
                <span className="text-yellow-600 font-medium">● {t('draft')}</span>
              )}
            </p>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm font-semibold text-gray-700">{t('patient')}:</label>
              <p className="text-gray-800">{documentReference.subject.display}</p>
            </div>

            <div>
              <label className="text-sm font-semibold text-gray-700">{t('encounter')}:</label>
              <p className="text-gray-800">{documentReference.context.encounter.display}</p>
            </div>

            <div>
              <label className="text-sm font-semibold text-gray-700">{t('custodian')}:</label>
              <p className="text-gray-800">{documentReference.custodian.display}</p>
            </div>

            <div>
              <label className="text-sm font-semibold text-gray-700 block mb-2">{t('summary')}:</label>
            ...
            <div>
              <label className="text-sm font-semibold text-gray-700 block mb-2">{t('clinicalFindings')}:</label>
      ...
      {!isFinalized && (
        <div className="card bg-yellow-50 border border-yellow-200">
          ...
          >
            {isFinalizing ? t('finalizing') : `✓ ${t('finalize')}`}
          </button>
        </div>
      )}
      ...
      {isFinalized && (
        <div className="card bg-green-50 border border-green-200">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-lg font-bold text-green-800">{t('finalized')}!</h3>
            </div>
          </div>
        </div>
      )}
